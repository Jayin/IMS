 ...
 * 
 */
public class ThreadPoolForShort {

	/* 单例 */
	private static ThreadPoolForShort instance = null;
	...
	// 实例new 出，也new 出线程池
	private ThreadPoolForShort() {
		isClose = false;
		threadPool = Executors.newCachedThreadPool();
	}

	public static synchronized ThreadPoolForShort getInstance() {
		if (instance == null)
			instance = new ThreadPoolForShort();
		return instance;
	}

	// 增加socket，就封装好任务，丢到线程池完成任务。接收发送数据
	public void add(Socket socket) {

		sockets.put(socketId, socket);
		// 当和socket断开的时候就是任务完成的时候
		threadPool.execute(new TaskForShort(socket, socketId, new TaskForShort.CallBack() {
